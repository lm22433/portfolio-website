...
export default function Projects() {
  ...
  return (
    <section
      id="projects"
      className="min-h-screen py-20 px-4 sm:px-6 lg:px-8 bg-[#2d2a2e]"
    >
      <div className="max-w-7xl mx-auto">
        {/* Section Header */}
        <div className="text-center mb-12 sm:mb-16">
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-[#ff6188] mb-4 tracking-tight px-4">
          ...
          <p className="text-sm sm:text-base md:text-lg lg:text-xl text-[#939293] max-w-2xl mx-auto px-4">
            ...
            <span className="text-[#fcfcfa]">Here are some of my recent projects that showcase my skills and
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {projects.map((project, index) => (
            <div
              key={project.title}
              className="ide-panel hover:border-[#ab9df2] transition-all duration-300 transform hover:-translate-y-2 flex flex-col"
            >
              {/* IDE-like header */}
              <div className="ide-panel-header">
                ...
                <span className="text-[#939293] text-xs ml-2 truncate">project_{index + 1}.tsx</span>
              </div>

              {/* Project Image */}
              <div className="relative h-40 sm:h-48 overflow-hidden bg-[#363337]">
                <img
                  src={project.image}
                  alt={project.title}
              ...
              <div className="p-4 sm:p-6 flex-1 flex flex-col">
                <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-[#ffd866] mb-3 break-words">
                  <span className="code-keyword text-xs sm:text-sm">function </span>
                  {project.title}
                  <span className="code-bracket">{'() {'}</span>
                </h3>
                <p className="text-sm sm:text-base text-[#939293] mb-4 flex-1">
                  <span className="code-comment">{'// '}</span>
                  {project.description}
                </p>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 sm:px-3 py-1 text-xs sm:text-sm bg-[#2d2a2e] border border-[#423f43] text-[#78dce8] rounded font-mono"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex gap-2 sm:gap-4">
                  <a
                    ...
                    className="flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 border border-[#a9dc76] text-[#a9dc76] rounded hover:bg-[#a9dc76] hover:text-[#2d2a2e] transition-all font-medium text-sm sm:text-base"
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 bg-[#ab9df2] text-[#2d2a2e] rounded hover:bg-[#c5b5ff] transition-all font-medium text-sm sm:text-base"
                  >
                    <ExternalLink className="w-3 h-3 sm:w-4 sm:h-4" />
                    Demo
                  </a>
                </div>
                <div className="mt-3 sm:mt-4 text-[#fcfcfa]">
                  <span className="code-bracket">{'}'}</span>
                </div>
              </div>
            </div>
